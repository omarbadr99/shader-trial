...
// BUILD: PROBE-1  TEMPORARY -- logs one diagnostic line to the console
// after 45 frames, then goes quiet. Swap back to OrganicO_PROD.tsx after.
//
...
/* TEMPORARY DIAGNOSTIC BUILD -- not the production component.
   Reports the real drawing-buffer size and the median frame cost once, after
   90 frames, then goes quiet. Nothing else differs from OrganicO_PROD. */
(function(){
  const t = []; let n = 0, lastT = performance.now(), done = false;
  const tick = () => {
    const now = performance.now();
    if (extA && extB && !PAUSED && !document.hidden){
      t.push(now - lastT); n++;
      if (!done && n >= 45){
        done = true;
        const s = t.slice(15).sort((a,b)=>a-b);
        const med = s[s.length>>1];
        console.info(
          "[Organic O] canvas " + canvas.width + "x" + canvas.height +
          " = " + (canvas.width*canvas.height/1e6).toFixed(2) + " Mpx" +
          " | css " + Math.round(innerWidth) + "x" + Math.round(innerHeight) +
          " | devicePixelRatio " + (devicePixelRatio||1) +
          " | quality(dpr) uniform " + EMBED_DPR +
          " | median frame " + med.toFixed(1) + " ms = " + (1000/med).toFixed(0) + " fps"
        );
      }
    }
    lastT = now;
